...
with torch.no_grad():
 for index,(info,args) in enumerate(cases()):
  ...
  for arm in ['P','S']:
   ...
   torch.testing.assert_close(val,vref,atol=2e-5,rtol=0.02)
   error[arm]={'max_abs':float((val-vref).abs().max()) if val.numel() else 0,'rel_L2':float(torch.linalg.vector_norm(val-vref)/torch.linalg.vector_norm(vref).clamp_min(1e-20))}
   del out,val
  del gold,vref
  ...
  (ROOT/(f'performance-{mode}-{o.repeat}.json')).write_text(json.dumps(results,indent=2))
